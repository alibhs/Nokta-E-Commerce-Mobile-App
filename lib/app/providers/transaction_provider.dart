...
class TransactionProvider with ChangeNotifier {
      ...
      if (response != null) {
        ...
        if (FlavorConfig.isAdmin()) {
          if (_detailTransaction!.account == null) {
            var data = await getAccountProfile.execute(
                accountId: _detailTransaction!.accountId);
      ...
      if (response.isNotEmpty) {
        ...
        await Future.forEach<Transaction>(_listTransaction, (element) async {
          if (element.account == null) {
            var data =
                await getAccountProfile.execute(accountId: element.accountId);
  ...
  accept() async {
    try {
      ...
      await loadDetailTransaction(
          transactionId: _detailTransaction!.transactionId);
      loadAccountTransaction();
  ...
  Future<void> submitReview(
      {required String transactionId, required List<Review> data}) async {
    try {
      _isLoading = true;
      notifyListeners();
      ...
      await changeStatus(
          transactionID: transactionId,
          status: TransactionStatus.yorumlandi.value);
  ...
  changeStatus({required String transactionID, required int status}) async {
    try {
      ...
      await changeTransactionStatus.execute(
