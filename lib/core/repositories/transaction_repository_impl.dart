...
class TransactionRepositoryImpl implements TransactionRepository {
  ...
  Future<void> acceptTransaction({required Transaction data}) async {
    await collectionReference.doc(data.transactionId).update({
      'transaction_status': TransactionStatus.tamamlandi.value,
    });
  }

  @override
  Future<void> addReview(
      {required String transactionId, required Review data}) async {
  ...
  Future<void> changeTransactionStatus(
      {required String transactionID, required int status}) async {
  ...
  Future<List<Transaction>> getAccountTransaction({
    ...
    if (data.docs.isNotEmpty) {
      temp.addAll(data.docs
          .map((e) => Transaction.fromJson(e.data() as Map<String, dynamic>)));
    }
